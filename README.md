...
- 合併看板：同時顯示 Funbox、玩具反斗城、M.M小舖與誠品線上商品
...
- `funbox_feed.json` / `toysrus_feed.json` / `mmtoyshop_feed.json` / `eslite_feed.json`：各來源的看板資料
- `feed.json`：四個來源合併後的看板資料
- `funbox_history.jsonl` / `toysrus_history.jsonl` / `mmtoyshop_history.jsonl` / `eslite_history.jsonl`：各來源最近 24 小時歷史
...
```bash
FUNBOX_NTFY_TOPIC="funbox-topic" TOYSRUS_NTFY_TOPIC="toysrus-topic" MMTOYSHOP_NTFY_TOPIC="mm-topic" ESLITE_NTFY_TOPIC="eslite-topic" python3 the_watcher.py
```
