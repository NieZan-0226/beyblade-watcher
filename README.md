...
- [M.M小舖 戰鬥陀螺](https://mmtoyshop.com/category/%F0%9F%8C%80%E6%88%B0%E9%AC%A5%E9%99%80%E8%9E%BA)
...
- 合併看板：同時顯示 Funbox、玩具反斗城、M.M小舖、誠品線上與森森文具玩具商品
...
- `mmtoyshop_tracked_items.json`：M.M小舖上次商品快照
- `eslite_tracked_items.json`：誠品線上上次商品快照
- `sensen_tracked_items.json`：森森文具玩具上次商品快照
- `funbox_feed.json` / `toysrus_feed.json` / `mmtoyshop_feed.json` / `eslite_feed.json` / `sensen_feed.json`：各來源的看板資料
- `feed.json`：五個來源合併後的看板資料
- `funbox_history.jsonl` / `toysrus_history.jsonl` / `mmtoyshop_history.jsonl` / `eslite_history.jsonl` / `sensen_history.jsonl`：各來源最近 24 小時歷史
...
```bash
FUNBOX_NTFY_TOPIC="funbox-topic" TOYSRUS_NTFY_TOPIC="toysrus-topic" MMTOYSHOP_NTFY_TOPIC="mm-topic" ESLITE_NTFY_TOPIC="eslite-topic" SENSEN_NTFY_TOPIC="sensen-topic" python3 the_watcher.py
```
...
$env:MMTOYSHOP_NTFY_TOPIC = "mm-topic"
