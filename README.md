...
Beyblade 商品上架、補貨、降價與下架偵測器，一次監控七個來源：
...
- [孩子玩伴 戰鬥陀螺](https://www.kidplaymate.com/collections/%E6%88%B0%E9%AC%A5%E9%99%80%E8%9E%BA)
- [MOMO 墊腳石 TAKARA TOMY / BEYBLADE](https://www.momoshop.com.tw/TP/TP0002451/main?brand=TAKARA%20TOMY&brandNo=20190717104950516&curPage=1)
- [MOMO Funbox TAKARA TOMY / Beyblade 戰鬥陀螺](https://www.momoshop.com.tw/categories/2186500000?brandName=TAKARA+TOMY&brandNoList=20190717104950516&brandSeriesStr=Beyblade+%E6%88%B0%E9%AC%A5%E9%99%80%E8%9E%BA%23%2320220916001674&has3P=y)

異動時會透過 [ntfy](https://ntfy.sh) 推播到手機，並產生一個合併的靜態網頁看板。七個來源各自保留狀態與歷史，其中一站暫時故障時，不會阻斷其他來源的檢查。
...
- 合併看板：同時顯示 Funbox、玩具反斗城、誠品線上、森森文具玩具、孩子玩伴、MOMO 墊腳石與 MOMO Funbox 商品
- 手機通知：標題保持短，完整商品名放在訊息第一行，避免 iOS 橫幅截斷後看不到補貨商品

## 檔案說明

- `the_watcher.py`：整合入口，一次執行七個監控器並合併看板資料
- `funbox_watcher.py`：Funbox 監控器
- `toysrus_watcher.py`：玩具反斗城監控器
- `eslite_watcher.py`：誠品線上策展頁監控器
- `sensen_watcher.py`：森森文具玩具監控器
- `kidplaymate_watcher.py`：孩子玩伴監控器
- `momo_watcher.py`：MOMO 墊腳石監控器，預設篩選 `BEYBLADE` 關鍵字
- `momo_funbox_watcher.py`：MOMO Funbox 監控器，預設篩選 `BEYBLADE|戰鬥陀螺` 關鍵字
- `momo_funbox_product_ids.json`：MOMO Funbox 額外固定監控的商品 ID 清單，用來補分類頁沒有列出的商品
...
- `kidplaymate_tracked_items.json`：孩子玩伴上次商品快照
- `momo_tracked_items.json`：MOMO 墊腳石上次商品快照
- `momo_funbox_tracked_items.json`：MOMO Funbox 上次商品快照
- `funbox_feed.json` / `toysrus_feed.json` / `eslite_feed.json` / `sensen_feed.json` / `kidplaymate_feed.json` / `momo_feed.json` / `momo_funbox_feed.json`：各來源的看板資料
- `feed.json`：七個來源合併後的看板資料
- `funbox_history.jsonl` / `toysrus_history.jsonl` / `eslite_history.jsonl` / `sensen_history.jsonl` / `kidplaymate_history.jsonl` / `momo_history.jsonl` / `momo_funbox_history.jsonl`：各來源最近 24 小時歷史
- `history.jsonl`：合併後的價格與庫存歷史
- `watchlist.json`：七站共用的關注關鍵字
- `index.html`：靜態網頁看板

## MOMO Funbox 混合式監控

MOMO Funbox 監控預設採用混合式：
...
```bash
FUNBOX_NTFY_TOPIC="funbox-topic" TOYSRUS_NTFY_TOPIC="toysrus-topic" ESLITE_NTFY_TOPIC="eslite-topic" SENSEN_NTFY_TOPIC="sensen-topic" KIDPLAYMATE_NTFY_TOPIC="kidplaymate-topic" MOMO_NTFY_TOPIC="momo-topic" MOMO_FUNBOX_NTFY_TOPIC="momo-funbox-topic" python3 the_watcher.py
```
...
$env:KIDPLAYMATE_NTFY_TOPIC = "kidplaymate-topic"
$env:MOMO_NTFY_TOPIC = "momo-topic"
$env:MOMO_FUNBOX_NTFY_TOPIC = "momo-funbox-topic"
py .\the_watcher.py
```
...
如果要標特定身分組，例如 `@陀螺鬥士`，不能直接填 `DISCORD_MENTION="@陀螺鬥士"`，Discord 會把它當一般文字。請改用身分組 ID：

```bash
DISCORD_ROLE_ID="123456789012345678"
```

或直接用 Discord role mention 格式：
...
1. Discord「使用者設定」→「進階」→ 打開「開發者模式」。
2. 到伺服器設定 → 身分組。
3. 對 `陀螺鬥士` 身分組按右鍵，選「複製身分組 ID」。
...
同時發到 Discord webhook，並標 `陀螺鬥士` 身分組：

```bash
DISCORD_WEBHOOK_URL="https://discord.com/api/webhooks/xxx/yyy" DISCORD_ROLE_ID="123456789012345678" NTFY_TOPIC="你的-topic" python3 the_watcher.py
```
...
```cron
*/1 * * * * cd /home/clmadmin/beyblade-watcher && DISCORD_WEBHOOK_URL="你的-discord-webhook-url-1,你的-discord-webhook-url-2" DISCORD_ROLE_ID="123456789012345678" NTFY_TOPIC=beyblade-x-k7m2qz FUNBOX_NTFY_TOPIC="funbox-beyblade-x-k9m4p7q2,beyblade-x-k7m2qz" /usr/bin/python3 the_watcher.py >> watcher-$(date +\%F).log 2>&1
```
...
如果 GitHub Actions 的 Discord 通知要標身分組，例如 `@陀螺鬥士`，新增 Secret：

- Name：`DISCORD_ROLE_ID`
- Secret：你的 `陀螺鬥士` 身分組 ID
...
Actions 執行後會將七站的狀態檔、歷史與合併 feed commit 回 `main`。
...
- `KIDPLAYMATE_NTFY_TOPIC`：僅孩子玩伴使用的 topic，會覆蓋 `NTFY_TOPIC`
- `MOMO_NTFY_TOPIC`：僅 MOMO 墊腳石使用的 topic，會覆蓋 `NTFY_TOPIC`
- `MOMO_FUNBOX_NTFY_TOPIC`：僅 MOMO Funbox 使用的 topic，會覆蓋 `NTFY_TOPIC`
- `DISCORD_WEBHOOK_URL`：Discord Webhook URL；未設定時不發 Discord，多個 URL 可用逗號分隔
- `DISCORD_USERNAME`：Discord Webhook 顯示名稱，預設 `TheWatcher`
- `DISCORD_MENTION`：Discord 訊息最上方要帶的 mention，例如 `@everyone` 或 `<@&角色ID>`；未設定時不標人
- `DISCORD_ROLE_ID`：Discord 身分組 ID，設定後會自動產生 `<@&角色ID>` 並真正 ping 該身分組；會優先於 `DISCORD_MENTION`
...
- `MOMO_FUNBOX_SHOP_URL`：MOMO Funbox TAKARA TOMY / Beyblade 戰鬥陀螺頁面網址
...
- `MOMO_VERIFY_SSL`：MOMO 商品 API 的 SSL 驗證，預設 `0`；若你的環境可正常驗證 MOMO 3PF 憑證，可設為 `1`
