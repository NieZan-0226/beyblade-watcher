...
Beyblade 商品上架、補貨、降價與下架偵測器，一次監控兩個來源：
...
異動時會透過 [ntfy](https://ntfy.sh) 推播到手機，並產生一個合併的靜態網頁看板。兩個來源各自保留狀態與歷史，其中一站暫時故障時，不會阻斷另一站的檢查。
...
- 合併看板：同時顯示 Funbox 與玩具反斗城商品

## 檔案說明

- `the_watcher.py`：整合入口，一次執行兩個監控器並合併看板資料
...
- `funbox_feed.json` / `toysrus_feed.json`：各來源的看板資料
- `feed.json`：兩個來源合併後的看板資料
- `funbox_history.jsonl` / `toysrus_history.jsonl`：各來源最近 24 小時歷史
- `history.jsonl`：合併後的價格與庫存歷史
- `watchlist.json`：兩站共用的關注關鍵字
- `index.html`：靜態網頁看板
...
```bash
FUNBOX_NTFY_TOPIC="funbox-topic" TOYSRUS_NTFY_TOPIC="toysrus-topic" python3 the_watcher.py
```
...
Actions 執行後會將兩站的狀態檔、歷史與合併 feed commit 回 `main`。
