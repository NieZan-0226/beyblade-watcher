#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Beyblade 上架 / 補貨偵測引擎（孩子玩伴 / Cyberbiz）。"""
...
SHOP_BASE = "https://www.kidplaymate.com"
COLLECTION_HANDLE = os.environ.get("KIDPLAYMATE_COLLECTION_HANDLE", "戰鬥陀螺")
COLLECTION_URL = os.environ.get(
    "KIDPLAYMATE_COLLECTION_URL",
    f"{SHOP_BASE}/collections/{quote(COLLECTION_HANDLE)}",
)
COLLECTION_API = os.environ.get(
    "KIDPLAYMATE_COLLECTION_API",
    f"{SHOP_BASE}/collections/{quote(COLLECTION_HANDLE)}/search_products.json",
)
REQUEST_COLLECTION_URL = quote(COLLECTION_URL, safe=":/?&=%#")
REQUEST_COLLECTION_API = quote(COLLECTION_API, safe=":/?&=%#")
SOURCE_NAME = "孩子玩伴"

NTFY_SERVER = os.environ.get("NTFY_SERVER", "https://ntfy.sh")
NTFY_TOPIC = os.environ.get("NTFY_TOPIC") or "kidplaymate-beyblade-k7m2qz"
...
STATE_FILE = os.environ.get("STATE_FILE", "kidplaymate_tracked_items.json")
FEED_FILE = os.environ.get("FEED_FILE", "kidplaymate_feed.json")
HISTORY_FILE = os.environ.get("HISTORY_FILE", "kidplaymate_history.jsonl")
...
HEADERS = {
    ...
    "Referer": REQUEST_COLLECTION_URL,
}


def fetch_products_page(page=1, per=24):
    params = {
        "page": page,
        "per": per,
    ...
    resp = requests.get(REQUEST_COLLECTION_API, headers=HEADERS, params=params, timeout=25)
    resp.raise_for_status()
    data = json.loads(resp.content.decode("utf-8"))
    if not isinstance(data, dict) or not isinstance(data.get("products"), list):
        raise RuntimeError("孩子玩伴分類 API 回傳格式不是商品清單。")
    return data


def fetch_products():
    first = fetch_products_page(1)
    products = list(first.get("products", []))
    total_pages = int(first.get("total_pages") or 1)
    for page in range(2, total_pages + 1):
        products.extend(fetch_products_page(page).get("products", []))
    return products
...
def image_url(raw):
    ...
    if isinstance(featured, dict):
        for size in ("grande", "large", "medium", "original"):
            if featured.get(size):
                return urljoin(SHOP_BASE, str(featured[size]))
    ...
    if isinstance(photos, list):
        for photo in photos:
            if isinstance(photo, dict):
                for size in ("grande", "large", "medium", "original"):
                    if photo.get(size):
                        return urljoin(SHOP_BASE, str(photo[size]))
    return ""
...
def parse_product(raw):
    ...
    return {
        "key": handle,
        "title": title,
        "url": urljoin(SHOP_BASE, f"/products/{handle}"),
        "price": parse_price(raw.get("first_variant_price") or raw.get("cheapest_variant_price")),
...
def main():
    print("開始檢查 孩子玩伴 Beyblade 上架 / 補貨…")
    ...
    try:
        ...
        if fail_count >= FAIL_ALERT_THRESHOLD and (fail_count - FAIL_ALERT_THRESHOLD) % FAIL_ALERT_THRESHOLD == 0:
            ntfy_publish(
                ...
                f"已連續 {fail_count} 次無法讀取孩子玩伴戰鬥陀螺清單。\n最後錯誤：{e}",
                tags=["warning"],
                priority=4,
            )
        return
